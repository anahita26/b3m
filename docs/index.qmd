...
author:
  - name: Anahita Darvish
    ...
    affiliation: 
      - name: Dept. of Mechanical Engineering, ETH Zurich
        url: https://mavt.ethz.ch/
      - name: Urban Climate Design Lab, NUS
        url: https://cde.nus.edu.sg/arch-ucdl/
format:
  html:
    toc: true
...
Everton Park, a typical high-rise, public residential complex in Singapore, was selected for this study. The complex consists of seven, 12-story-high buildings facing north or south, in the configuration shown in @fig-layout. While the full residential complex was represented in the simulation to capture inter-building effects, detailed analysis was focused on three representative buildings—Blocks 2, 5, and 7—which was sufficient to assess the impact of passive retrofit measures across the site. Blocks 2 and 5 each contain 94 residential flats, while the central building, Block 7, contains 120 flats due to its different apartment configuration. In contrast to single-building studies, evaluating multiple blocks allows for a more robust simulation of energy retrofit measures (ERMs), as they will be applied to buildings with different relative positions, solar exposure, and boundary conditions.

![Studied residential buildings at Everton Park in Singapore, a typical high-density housing complex in the Bukit Merah planning area](images/everton_park_detailed.png){#fig-layout}

Energy consumption in each building was modeled using EnergyPlus, a widely-used physics based building energy modelling program [@singEnergyefficientRetrofittingMultistorey2021]. Baseline building geometries were derived from previously developed 3D models reported by Xu et al. [@XU2022103775], which represent the Everton Park buildings using a simplified multi-story modeling approach based on EnergyPlus multiplier objects. For the surrounding buildings (Blocks 2 and 5), typical floors were modeled using floor multipliers, with the top floor modeled separately to account for roof solar exposure. Within each floor, distinct apartment types were represented using unit-level multipliers, while end units were modeled explicitly due to their unique facade exposure. The central building (Block 7), which has a different architectural layout, followed a similar strategy but employed a different multiplier configuration to reflect its distinct apartment composition (see Appendix A: @fig-aptlayouts and @fig-model).

In all buildings, only bedrooms were modeled as conditioned thermal zones and merged into a single thermal zone per flat, while living rooms, kitchens, and bathrooms were treated as non-thermal zones. This assumption reflects typical air-conditioning usage patterns in Singaporean public housing and directly influences the magnitude of simulated cooling demand. Air-conditioning operation was restricted to nighttime sleeping hours, with a temperature setpoint of 25 °C applied from 23:30 to 06:00, consistent with national recommendations and observed residential usage patterns in Singapore. During daytime hours, air-conditioning was assumed to be off, and natural ventilation was enabled by opening windows in all thermal zones. All building models share identical internal loads, schedules, air-conditioning settings, and construction materials, and differ only in apartment composition, facade geometry, and relative site placement. Construction assemblies, material properties, and operational schedules were adopted directly from Xu et al. and are summarized in Appendix A @fig-construction, @fig-appliances, and @fig-activities.
...
To determine a baseline cooling demand for each block, the existing building models were simulated with the developed microclimate weather dataset. All simulation parameters are consistent with those used by Xu. et al., with important parameters presented in [@tbl-parameters; @XU2022103775].
...
parameters_df <- tibble(
  `Parameter` = c("Simulation period", "Days considered", "Reporting frequency", "Coefficient of performace (COP)", "Temperature Setpoint "),
  `Value` = c("June 2015", "Weekdays", "Daily", "3", "25 deg C")
)
parameters_df |> 
  gt() |> 
  cols_align("left", everything()) |> 
...
@eq-ac was applied to calculate upper and lower bounds for the monthly AC electricity consumption of each building. Variables include: $w$ , the fraction of weekdays in a typical month (\~0.73), accounting for weekday-only occupancy patterns assumed in the simulations due to lack of published information on resident activities during the weekends, $p_{AC}$ , the percentage of total monthly electricity consumption per household used for cooling, $E_{tot}$ , the total monthly electricity consumption per household, and $N_{flats}$ , the total number of flats in each residential building.

$$
\ E_{AC} = w * p_{AC} * E_{tot} * N_{flats}
$$ {#eq-ac}

@fig-reference shows the range of reference values calculated and where each baseline model falls within the range. All baseline values were found to fall within benchmark ranges.
...
The data shows that coatings on the roof have a much larger impact than coatings on the exterior walls. For example, block 5 showed 1.34% savings due to exterior wall coatings compared to 3.5% savings due to roof coating. This is expected, as the roof receives higher cumulative solar irradiance in Singapore’s climate, leading to greater solar heat gains and elevated surface temperatures. Reducing roof absorptance therefore lowers conductive heat transfer into the building and results in a more pronounced reduction in cooling demand compared to vertical wall surfaces. In both cases, blocks 2 and 5 demonstrate a similar percentage cooling savings, while block 7 performs best. Since the simulations were run under identical climatic and modelling conditions, this difference in response is likely due to building geometry. A more in depth analysis is needed to verify the cause of block 7's higher cooling energy savings.
...
To add a GR to the baseline model, a new vegetation material was created, followed by a new construction. The new construction was then applied to the exterior roof, following a similar setup to the cool coatings. For this study, only the leaf area index (LAI) and plant height were varied between cases. All other parameters related to the vegetation material, such as substrate thickness and soil conductivity, were set to the default EnergyPlus values. Two cases of GRs were simulated with details presented in @tbl-details. The two cases represent distinct green roof typologies: a grass-based system characterized by dense, continuous vegetation (LAI = 3) with limited plant height, and a shrub-based system with sparser horizontal coverage (LAI = 1.5) but greater vertical coverage. These parameter choices reflect typical extensive green roof configurations [@yaghoobian2015] and allow the comparison of two different mechanisms within a simplified modelling framework.
...
All green roof cases include coated exterior walls (absorptance 0.2) for simulation. @fig-comparison shows a comparison between the grass roof, shrub roof, and coated roof. Both GR cases performed similarly across all blocks, with around 3-4% cooling energy savings, and grass demonstrating slightly better savings than shrubs. When comparing the GRs to the coated roof, the coated roof showed more energy savings across all blocks (\~5-10%). This suggests that, under Singapore's hot and humid climate, direct reduction of solar heat absorption through increased reflectance is more effective at lowering cooling demand than evapotranspiration-based cooling mechanisms.

```{r}
#| echo: false
#| label: fig-comparison
#| fig-cap: Comparison of green roof cases to cool-coated roof
library(here)
library(tidyverse)
library(ggplot2)
library(dplyr)
...
The high reflectance coatings showed high-moderate impact on cooling energy savings, primarily because the roof experiences substantial solar heat gains, making reductions in absorptance particularly effective at lowering cooling demand. This trend was consistent across all blocks, indicating that material-based strategies are most effective for this site. Green roofs showed moderate cooling savings across all blocks but are secondary to reflective coatings. Overhang shading attached to the exterior windows showed limited additional benefit in all blocks given the existing building design. Although the absolute cooling savings varies by block, the relative ranking of ERMs is stable, suggesting that the results are robust across buildings within the same residential complex. For example, although block 7 showed a higher cooling reduction capacity in all metrics, cool coating remained the most effective measure for each block, followed by roof vegetation and overhang shading.

While ERMs with high energy-saving potential emerge as good technical candidates for facade retrofit, energy performance alone is insufficient to determine implementation priority. In practice, retrofit decisions require a cost–benefit analysis that integrates energy savings with economic, structural, and maintenance considerations. Such an analysis is essential to identify measures that are not only technically effective but also economically viable.
...
erms <- c("Cool roof +  cool walls (7)", 
          "Cool roof +  cool walls (2,5), Green roof + cool walls (2,7,5)", 
          "Cool walls (2,7,5)",
          "Shading (2,7,5)")
energy_df <- tibble(
...
Future work could expand on the number of passive ERMs tested. Selecting more ERMs from each category could deepen category-level conclusions in a given climate. In addition, a future study could investigate more blocks in the same residential complex or multiple complexes in the same district. Scaling up multi-building retrofit analyses will support decision making at the neighborhood or district level and improve the generalizability of retrofit performance. Finally, integrating a cost–benefit analysis that couples energy savings with capital, installation, and maintenance costs would enable performance-based results to be translated into actionable retrofit decisions, particularly for large-scale residential housing with constrained budgets.

# Appendix A: Baseline Model Details {.appendix .unnumbered}

![Typical floor plans for (a) 3-room and 5-room flats in the surrounding buildings (Buildings 1–6), and (b) two types of 3-room flats in the central building (Building 7). Reproduced from Xu et al. [@XU2022103775].](images/typical_floor_plan.png){#fig-aptlayouts}

![Modelling configurations: (a) 3D view of target buildings; application of multipliers to (b) the surrounding buildings with 3-room (3R) and 5-room (5R) flats; and (c) the central building with two types of 3-room flats (3R-a and 3R-b). End flats are colored in dark blue and modeled separately. Reproduced from Xu et al. [@XU2022103775].](images/modelling_configs-03.png){#fig-model}

![Construction materials of the modeled buildings. Reproduced from Xu et al. [@XU2022103775].](images/construction_mat-01.png){#fig-construction}

![Household electrical appliance assumptions used in the baseline models. Reproduced from Xu et al. [@XU2022103775].](images/operations-01.png){#fig-appliances}

![Resident activity assumptions used in the baseline models. Reproduced from Xu et al. [@XU2022103775].](images/resident_activities-01.png){#fig-activities}
