...
Energy consumption in each building was modeled using EnergyPlus, a widely-used physics based building energy modelling program [@singEnergyefficientRetrofittingMultistorey2021]. Baseline building geometries were derived from previously developed 3D models reported by Xu et al. [@XU2022103775], which represent the Everton Park buildings using a simplified multi-story modeling approach based on EnergyPlus multiplier objects. This approach enables repeated floors and dwelling units with similar layouts to be represented efficiently, while preserving key geometric and solar-exposure differences across the complex. For the surrounding buildings (Blocks 2 and 5), typical floors were modeled using floor multipliers, with the top floor modeled separately to account for roof solar exposure. Within each floor, distinct apartment types were represented using unit-level multipliers, while end units were modeled explicitly due to their unique facade exposure. The central building (Block 7), which has a different architectural layout, followed a similar strategy but employed a different multiplier configuration to reflect its distinct apartment composition (see Appendix A: @fig-aptlayouts and @fig-model).
