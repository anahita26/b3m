...
results <- read_csv(here("data", "results", "all_scenarios_blk5"))
results_greenroof <- read_csv(here("data", "results", "greenroof_data_blk5"))

baseline <- read_csv( here("data", "results", "blk5_baseline"))
...
results |> 
  ...
  facet_wrap(~facade) +
  labs(
    title = "Basline vs Retrofit Scenarios Blk5",
    x = "Scenario",
...
results_pct |> 
  ...
  facet_wrap(~facade) +
  labs(
    title = "Percent Change in Cooling Demand vs Baseline Blk5",
    x = "Scenario",
    y = "Δ Cooling Demand (%)"
  )
...
results_greenroof_pct |> 
  ...
  labs(
    title = "Percent Change in Green Roof Effects Blk5",
    x = "Case",
    y = "Δ Cooling Demand (%)"
  )
